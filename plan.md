...
### Section chaining and execution context

- **`ctx.node_outputs` is the authoritative in-memory execution state.** During a full run, section output propagates only via the return value of `run_section()`; the executor never refills `ctx.node_outputs` from storage in the main loop. Storage is for persistence only, not execution state.
- **Section chaining depends on `{{previous_sections}}`.** Each section’s prompt is given a `previous_sections` value built from `ctx.node_outputs` for prior section ids. Prompts must explicitly include `{{previous_sections}}` in their template to receive prior content; otherwise context flow is broken and later sections may get empty or placeholder output.
- **Guardrails:** Before running a node, the executor checks that any declared `depends_on` ids exist in `ctx.node_outputs`. Prompt validation (`validate_template_prompts(template_id)`) runs at the start of each run and logs warnings if a section has dependencies or is not the first section but its prompt file does not reference `{{previous_sections}}`. Regression tests in `tests/test_executor_section_chaining.py` verify that section output propagates and that the executor uses the return value of `run_section()`, not storage, for chaining.
- **Template validation:** Run `validate_template_prompts(template_id)` to check prompt files exist, dependency chain is valid, and prompts that need prior context contain `{{previous_sections}}`. This is invoked automatically when a run starts; it can also be called as a utility (e.g. from a CLI or when loading templates). Example: `from src.orchestration import validate_template_prompts; validate_template_prompts("hld")`. Warnings are logged only; generation is not failed.
